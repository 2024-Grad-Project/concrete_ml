...
class Counter:
    ...
    def inc(image):
        output = quantized_module.forward(image)
        indices = output if isinstance(output, np.ndarray) else output.detach().cpu().numpy()
        return indices
...
CounterFhe = Counter.compile({"inc": [iamage_input.numpy()], "dec": [(4, 3)]})
x_enc = CounterFhe.inc.encrypt(iamage_input.numpy())
