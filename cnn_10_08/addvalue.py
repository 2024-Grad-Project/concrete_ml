...
class ImprovedCNN(nn.Module):
    ...
    def forward(self, x, add_value=1):
        if x is None:
        ...
        return self.get_approximate_argmax(activations, scale_factor=10, add_value=add_value)

    def get_approximate_argmax(self, activations, scale_factor=10, add_value=1):
        ...
        return approximate_indices.round() + add_value
...
torch_inputset2334 = [(sample_input.cpu(), torch.tensor(5, dtype=torch.int32).cpu())]
N_FEAT = 12


torch_inputset = [(sample_input, torch.tensor(5, dtype=torch.float32))]
...
output = q_module.forward(np_array, 5)
print(output)
print("here is after forward")

#fhe_directory = '/home/giuk/zama_fhe_directory/test_2/' # 자기 자신에 맞게 파일명 바꾸기
