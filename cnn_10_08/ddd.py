...
class ImprovedCNN(nn.Module):
    ...
    def get_approximate_argmax(self, activations, scale_factor=10):
        ...
        return (approximate_indices.round()+1)*8
...
@fhe.module()
class MLWithRandomMultiplication:
    def __init__(self):
        self.ml_model = net  # net을 직접 참조

    @fhe.function({"self": "clear", "x": "encrypted"})
    def predict(self, x):
        return self.ml_model(x)

    @fhe.function({"self": "clear", "x": "encrypted", "random_factor": "clear"})
    def multiply_with_random(self, x, random_factor):
        return x * random_factor

    composition = fhe.Wired([
        fhe.Wire(fhe.Output(predict, 0), fhe.Input(multiply_with_random, 0))
    ])

# FHE 모듈 생성
fhe_module = MLWithRandomMultiplication()
...
q_module = compile_torch_model(
    torch_model= fhe_module , 
    torch_inputset=(sample_input,23), 
    import_qat=False,
...
#fhe_directory = '/home/giuk/zama_fhe_directory/test_4/' # 자기 자신에 맞게 파일명 바꾸기
