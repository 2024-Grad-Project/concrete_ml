...
class CustomResNet50(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
         ...
         # 이 부분을 대체하여 Slice 연산을 피함
        N, C, H, W = x.shape
        x = x.view(N, C, H * W)  # flatten 대신 슬라이싱을 피하면서 flatten
        x = x.sum(dim=-1) / (H * W)  # 평균을 내기 위한 sum
...
fhe_directory = '/home/giuk/fhe_client_server_files3/' # 자기 자신에 맞게 파일명 바꾸기
