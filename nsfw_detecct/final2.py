...
class CustomResNet50(nn.Module):
    ...
    def forward(self, x, y=1.0):
        ...
        heshoutput = self.hash_function(indices, y, z)
    ...
    def hash_function(self, indices, y, z):
        # Step 1: Scale inputs
        scaled_indices = indices * 0.1
        scaled_y = y * 0.2
        scaled_z = z * 0.3

        # Step 2: Combine using Add
        combined = scaled_indices + scaled_y + scaled_z

        # Step 3: Apply supported non-linear transformation (e.g., Tanh)
        transformed = torch.tanh(combined)  # Replace Sin/Cos with Tanh for compatibility

        # Step 4: Aggregate using ReduceSum
        hashed = transformed.sum(dim=1, keepdim=True)
        return hashed
