...
class CustomResNet50(nn.Module):
    ...
    def forward(self, x, y=1.0):
        ...
        indices = self.approximate_argmax(activations)
        heshoutput = self.approximate_hash_function(indices, y, z)
        #heshoutput = indices*y
        return indices, heshoutput  # INT64 -> FLOAT 변환 추가
    
    def approximate_argmax(self, activations):
        # Step 1: 각 행의 최대값 계산
        max_values = activations[:, 0:1]  # 첫 번째 값을 초기 최대값으로 설정
        max_values = torch.max(max_values, activations[:, 1:2])
        max_values = torch.max(max_values, activations[:, 2:3])
        max_values = torch.max(max_values, activations[:, 3:4])
        max_values = torch.max(max_values, activations[:, 4:5])
        max_values = torch.max(max_values, activations[:, 5:6])
        max_values = torch.max(max_values, activations[:, 6:7])
        max_values = torch.max(max_values, activations[:, 7:8])
        max_values = torch.max(max_values, activations[:, 8:9])
        max_values = torch.max(max_values, activations[:, 9:10])
        # Step 2: 최대값 위치 확인
        is_max = torch.eq(activations, max_values).float()  # 최대값 위치를 1로 설정, 나머지는 0
        ...
        weighted_indices = is_max * index_matrix  # (batch_size, 10)

        # Step 5: ReduceSum으로 최종 인덱스 계산
        final_index = torch.sum(weighted_indices, dim=1)  # (batch_size,)
