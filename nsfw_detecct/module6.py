...
class Counter:
    ...
    def classify(image):
        ...
        # quantized_module에 전달하기 전에 데이터 타입 확인
...
# 입력 데이터 준비시 데이터 타입 명시적 지정
input_data = iamage_input.numpy().astype(np.float32)

# 컴파일 설정에 데이터 타입 관련 설정 추가
config = Configuration(
    enable_unsafe_features=True,
    use_insecure_key_cache=True,
    insecure_key_cache_location="~/.cml_keycache",
    default_precision=32  # 정밀도 명시적 지정
)

# 컴파일 실행
CounterFhe = Counter.compile(
    {"classify": [input_data], "mul": [(4, 3)]},
    configuration=config
)
