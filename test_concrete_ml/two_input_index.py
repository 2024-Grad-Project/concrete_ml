...
class ImprovedCNN(nn.Module):
    ...
    def forward(self, x,y=1.0):
        ...
        indices = self.get_approximate_argmax(activations)
        hey = indices*y
        return hey

    def get_approximate_argmax(self, activations, scale_factor=10):
        exponents = (activations * scale_factor)
...
my_inputs_encryption_status=('encrypted', 'clear')
constant_input = torch.tensor([[10.5]])  # example constant
constant_input2 = torch.tensor([[10.5]], dtype=torch.float32)
q_module = compile_torch_model(
    torch_model=net, 
    torch_inputset=(sample_input, constant_input2), 
    import_qat=False,
    ...
    global_p_error = None,
    verbose=False,
    inputs_encryption_status = my_inputs_encryption_status,
...
output = q_module.forward(np_array, constant_input2.numpy())
print(output)
print("here is after forward")
#print(concrete.ml.__version__)
#fhe_directory = '/home/giuk/zama_fhe_directory/test_9_two_input/' # 자기 자신에 맞게 파일명 바꾸기
